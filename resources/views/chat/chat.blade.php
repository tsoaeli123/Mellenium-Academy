...
<div class="container">
  ...
  <div class="sidebar">
    <h3><a href="{{route('student.dashboard')}}"><i class="fas fa-home"></i></a>  Chats Live</h3>
...
window.Chat = {
    openChat(id, name){
        ...
        fetch(`/messages/${id}/seen`, {method:'POST', headers:{'X-CSRF-TOKEN':csrf}});

        fetch(`/messages/${id}`).then(r=>r.json()).then(msgs=>{
...
if (replyBox && replyBox.dataset.replyId) {
    ...
    delete(id){
        ...
        fetch('/messages/'+id, {method:'DELETE', headers:{'X-CSRF-TOKEN':csrf}});
...
document.getElementById('messageInput').addEventListener('input', ()=>{
    if(selectedUserId)
        fetch(`/messages/${selectedUserId}/typing`, {method:'POST', headers:{'X-CSRF-TOKEN':csrf}});
});
    ...
    window.Echo.private(`chat.${currentUser.id}`)
        .listen('MessageSent', (e)=>{
            ...
            if(selectedUserId===m.sender_id){
                ...
                Chat.scrollToBottom();
                fetch(`/messages/${m.sender_id}/seen`, {method:'POST', headers:{'X-CSRF-TOKEN':csrf}});
