...
<div class="main-content">
    <!-- Header -->
    <div class="header">
        <h1><i class="fas fa-user-graduate"></i> Student Courses</h1>
        <div class="user-info">
    ...
    <div class="card">
        ...
        <div class="quick-actions">
            @foreach($enrol as $rol)
            @if($rol->payment_status != "pending")
            @foreach($course as $cs)
            
            @if($cs->id == $rol->subject_id)
            <a href="{{route('student.showCourse', $cs->title)}}" class="action-btn">
                <div class="action-icon">
                    <img src="{{$cs->thumbnail}}" style="border-radius:50%; border: 2px solid darkgreen;" width="100"/>
                </div>
                <div><b>{{$cs->title}}</b></div>
            </a>
            @endif
            @endforeach
            @endif
